      ...
      if (students?.length > 0) {
        ...
        for (let student of students) {
          ...
          row.innerHTML = `
                ...
                <td class="text-center"><span style="color: ${student.gender == 'M' ? '#10559A' : '#DB4C77'}" class="fas fa-child"></span></td>
...
async function deEnrollStudentByCourse(courseName, studentId) {
  const endpoint = `http://localhost:3000/courses/${courseName}/students/${studentId}`
  try {
...
async function getStudentsByCourse(courseName) {
  const endpoint = `http://localhost:3000/courses/${courseName}/students`
  try {
    const res = await fetch(endpoint, {
      method: 'GET'
    })
