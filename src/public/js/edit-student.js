...
async function updateStudent(student, id) {
	const endpoint = `https://sigerd-api.herokuapp.com/students/${id}`;
	try {
	...
	try {
		if (ev.target.dataset?.parentTitle) {
			...
			const parents = await getParent(
				formEditStudent.dataset.studentId,
				traduction[btn.dataset.parentTitle]
			);
...
parentForm.addEventListener("submit", async function (ev) {
	...
	try {
		...
		const feedback = await updateParent(
			formEditStudent.dataset.studentId,
			parent,
			traduction[this.dataset.parent]
		);
...
async function getParent(studentId, parentPath) {
	const endpoint = `https://sigerd-api.herokuapp.com/students/${studentId}/${parentPath}`;
	try {
		const res = await fetch(endpoint, {
			method: "GET",
		});
